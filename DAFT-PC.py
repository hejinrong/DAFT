...
parser.add_argument("-s","--shot_num_per_class",type = int, default = 1)  #meta training shot num
...
parser.add_argument("-z","--test_lsample_num_per_class",type=int,default=5, help='5 4 3 2 1') #meta testing shot num
...
class D_Res_3d_CNN(nn.Module):
    ...
    def forward(self, x): #support_1shot:(9, 100, 9, 9)
        x = x.unsqueeze(1) #support_1shot:(9, 1, 100, 9, 9)  (num_shot*num_class = 9, num_channel, L, H, W)
        x = self.block1(x) #support_1shot:(9, 8, 100, 9, 9)
        x = self.maxpool1(x) #support_1shot:(9 ,8, 25, 5, 5)
        x = self.block2(x) #support_1shot:(9, 16, 25, 5, 5)
        x = self.maxpool2(x) #support_1shot:(9, 16, 7, 3, 3)
        x = self.conv(x) #support_1shot:(9, 32, 5, 1, 1)
        x = x.view(x.shape[0],-1) #support_1shot:(9, 160)
        # y = self.classifier(x)
        return x

class D_Res_3d_muti_CNN(nn.Module):
    def __init__(self, in_channel, out_channel1, out_channel2, out_channel3):
        super(D_Res_3d_muti_CNN, self).__init__()

        self.block1_1 = residual_block(in_channel, out_channel1)
        self.maxpool1_1 = nn.MaxPool3d(kernel_size=(4, 2, 2), padding=(0, 1, 1), stride=(4, 2, 2))
        self.maxpool1_2 = nn.MaxPool3d(kernel_size=(4, 2, 2), stride=(4, 2, 2), padding=(2, 1, 1))
        self.conv1 = nn.Conv3d(in_channels=out_channel2, out_channels=32, kernel_size=3, bias=False)

        self.block2_1 = residual_block(in_channel,out_channel1)
        self.maxpool2_1 = nn.MaxPool3d(kernel_size=(4,2,2),padding=(0,1,1),stride=(4,2,2))
        self.block2_2 = residual_block(out_channel1,out_channel2)
        self.maxpool2_2 = nn.MaxPool3d(kernel_size=(4,2,2),stride=(4,2,2), padding=(2,1,1))
        self.conv2 = nn.Conv3d(in_channels=out_channel2,out_channels=32,kernel_size=3, bias=False)
...
class Network(nn.Module):
    ...
    def forward(self, x, domain='source'):  # x
        # print(x.shape)
        if domain == 'target':
            x = self.target_mapping(x)  # (9, 100, 9, 9)
        elif domain == 'source':
            x = self.source_mapping(x)  # (9, 100, 9, 9)
        # print(x.shape)#torch.Size([45, 100, 9, 9])
        feature = self.feature_encoder(x)  # (9, 160)
        output = self.classifier(feature)   # (9, 9)
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(10000):
        ...
        try:
            source_data, source_label = source_iter.next() #source_data: torch.Size([128, 128, 9, 9])
        except Exception as err:
            source_iter = iter(source_loader)
            source_data, source_label = source_iter.next()

        try:
            target_data, target_label = target_iter.next() #target_data: torch.Size([128, 102, 9, 9])
        except Exception as err:
            target_iter = iter(target_loader)
        ...
        if episode % 2 == 0:
            ...
            # sample datas
            supports, support_labels = support_dataloader.__iter__().next()  # source domain supports: torch.Size([9, 128, 9, 9])  1shot
            querys, query_labels = query_dataloader.__iter__().next()  # source domain querys: torch.Size([171, 128, 9, 9])  1shot

            # calculate features
            support_features, support_outputs = feature_encoder(supports.cuda())  # 1shot_support_features: torch.Size([9, 160])   support_outputs：torch.Size([9, 9])
            query_features, query_outputs = feature_encoder(querys.cuda())  # 1shot_query_features: torch.Size([171, 160])
            target_features, target_outputs = feature_encoder(target_data.cuda(), domain='target')  # 1shot_target_features: torch.Size([128, 160])

            # Prototype network
            if SHOT_NUM_PER_CLASS > 1:
                support_proto = support_features.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS, -1).mean(dim=1)  # 1shot_(9, 160)
            else:
        ...
        # target domain few-shot + domain adaptation
            ...
            # sample datas
            supports, support_labels = support_dataloader.__iter__().next()  # 1shot_target domain supports: torch.Size([9, 102, 9, 9])
            querys, query_labels = query_dataloader.__iter__().next()  # 1shot_target domain querys: torch.Size([171, 102, 9, 9])

            # calculate features
            support_features, support_outputs = feature_encoder(supports.cuda(),  domain='target')  # 1shot_target domain support_features: torch.Size([9, 160])
            query_features, query_outputs = feature_encoder(querys.cuda(), domain='target')  # 1shot_target domain query_features: torch.Size([171, 160])
            source_features, source_outputs = feature_encoder(source_data.cuda()) #source_features: torch.Size([128, 160])
            ...
            '''domain adaptation'''
            features = torch.cat([support_features, query_features, source_features], dim=0)  #domain features: torch.Size([308, 160])
        ...
        if (episode + 1) % 1000 == 0 or episode == 0:
            ...
            train_datas, train_labels = train_loader.__iter__().next()
            train_features, _ = feature_encoder(Variable(train_datas).cuda(), domain='target')  # 5shot_meta testing train_features:torch.size(45, 160)
